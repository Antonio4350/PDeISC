...
export default function PerfilScreen() {
  ...
  const elegirImagen = async () => {
    ...
    if (status !== "granted") return Alert.alert("Permiso requerido", "Necesitas permitir acceso a tus imágenes.");
  ...
  const guardarPerfil = async () => {
    ...
    if (!/^\d+$/.test(telefono) || telefono.length < 8) return setMensaje("El teléfono debe tener solo números (mínimo 8 dígitos).");

    try {
      const res = await fetch(`${API_URL}/perfil`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: user, nombre, direccion, telefono, documento, foto_perfil: imagen }),
      });
      const data = await res.json();
      if (data.success) setMensaje("Perfil actualizado correctamente.");
      else setMensaje(data.error || "Error al guardar perfil.");
    } catch {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity onPress={elegirImagen}>
        <Image source={{ uri: imagen || "https://cdn-icons-png.flaticon.com/512/3135/3135715.png" }} style={styles.avatar} />
