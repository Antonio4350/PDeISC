...
function startGame() {
  ...
  document.getElementById("currentMode").textContent = `Modo: ${
    mode === "player" ? "1 vs 1" : "vs Máquina"
  }`;
  updateRanking();
  updateTurn();
  audio.currentTime = 0;
...
function updateTurn() {
  if (mode === "player") {
    document.getElementById("turnDisplay").textContent = `Turno: ${
      playerTurn === 1 ? names.player1 : names.player2
    }`;
  } else {
    document.getElementById(
      "turnDisplay"
    ).textContent = `${names.player1}, elegí tu jugada.`;
  }
}
